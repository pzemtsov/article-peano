public class Binary
{
    ...
    public static Binary TWO = newBinary (false, ONE);
    ...
    private Binary (boolean d, Binary next)
    {
        this.lowest = d;
        this.next = next;
    }

    private Binary (boolean d)
    {
        this.lowest = d;
        this.next = null;
    }

    private static Binary newBinary (boolean d, Binary next)
    {
        if (next != null) {
            if (d) {
                if (next.link1 == null) {
    ...
    private static CompareResult compare (Binary x, Binary y)
    {
        ...
        return x.lowest == y.lowest ? CompareResult.EQU : x.lowest? CompareResult.GT : CompareResult.LT;
    }
    ...
    {
        if (s == null) {
            s = !lowest ? newBinary (true, next) :
                  next == null ? TWO : newBinary (false, next.S ());
        }
        return s;
    }
    ...
    {
        if (lowest)
            return next == null ? null : newBinary (false, next);
        if (next == null)
            throw new IllegalArgumentException ("Negative number");
        return newBinary (true, next.D0());
    }
    ...
    private static Binary sub_borrow (Binary x, Binary y)
    {
        ...
        return (s == null && !d) ? null : newBinary (d, s);
    }
    ...
    {
        return isZero() ? this : newBinary (false, this);
    }
